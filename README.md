...
| `polygon`  | 観測可能領域を凸包で近似し、GeoJSON 形式のポリゴンとして出力 |
...
Rust 1.91.1（`rust-toolchain.toml` で固定）以上が必要。
...
- **ポリゴン生成**: 富士山中心の方位帯ごとに、二分法で「太陽高度＝富士高度±閾値」となる距離を解き、観測候補点群の凸包を取る over-approximation
